...
router.post('/login', (req, res)=>{
  ...
  const definedEmail = 'school@email.com';
  const definedPassword = 'school123';

  const userEmail = req.body.email;
  const userPassword = req.body.password;

  try {
    if(userEmail === definedEmail && userPassword === definedPassword){
      return res.status(200).send({success: true, result:"SUCCESS"});
    }else{
      return res.status(400).send({success: false, result: "UNAUTHORIZED USER"});
    }
  } catch (error) {
    return res.status(501).send({success: false, result: "FAILED TO LOGIN"});
  }
});
// ===================================================================================================
  ...
  try {
    ...
    if (req.user.type === "admin") {
      ...
      user = await User.findByIdAndUpdate(req.params.id, {
        ...
        type: type ? type : existingUser.type
      }, { new: true });
    } else {
